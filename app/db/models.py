...
class Discussion(TimestampMixin):
    ...
    def add_user_mentions(self, mention):
        self._user_mentions.add(mention)
    ...
    async def discussion_participant_relation(self, participant_id: uuid.uuid4):
        await DiscussionParticipantRelation(discussion_id=self.id,
                                            ...
                                            inviter_id=participant_id).create()
...
class DiscussionParticipantRelation(TimestampMixin):
    ...
    participant_id = db.Column('participant_id', UUID(),
                               db.ForeignKey('users.id', ondelete="CASCADE"), nullable=False)
    inviter_id = db.Column('inviter_id', UUID(),
                           db.ForeignKey('users.id', ondelete="CASCADE"), nullable=False)
    ...
    @participant.setter
    def set_participant(self, _participant):
    ...
    @inviter.setter
    def set_inviter(self, _inviter):
        self._inviter = _inviter
...
class DiscussionComment(TimestampMixin):
    ...
    @replied_message.setter
    def set_replied_message(self, _replied_message):
        self._replied_message = _replied_message
